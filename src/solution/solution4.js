...
const Solution=({n1,n,ref0, ref1,setk,k, page, setp, dim, setm, modal, setl, setn, setdim, tab, setErrorr})=>{
    ...
    const [v1, setv1]=useState(((k*1)+(n*1)**2))
    const {logouter}=useLogout()
    const [v, setv]=useState(((k*1)+(n*1)**2))
    const title= "Variable Coefficient Differential Equations 3"
    const sv1='{'+ math.round(math.sqrt(math.abs(v))*10**3)/10**3+'i}'
    const sv2='i'
    const sv3='{}'
    const sv4='{'+  math.round(math.sqrt(math.abs(v))*10**3)/10**3+'}'
    const [sv, setsv]=useState(sv4)
    useEffect(()=>{
        setn(n1*1); setk(Number(k));
        setv(math.abs(v)); setv1(((k*1)+(n*1)**2));
        if (v1<0 && v1!==-1){
            setsv(sv1)
        } else if (v1<0 && v1===-1){
            setsv(sv2)
    ...
    const p= (n,x)=> {
        ...
        if (v1<0){
            ...
            if (n==0){
                ...
                Se= (math.sqrt(1-x**2)**(-1))*S
            } else if (n==2){
                Se=(math.sqrt(1-x**2)**(-3))*(x*S-math.sqrt(v*(1-x**2))*C)
            } else if (n==3){
                Se=(math.sqrt(1-x**2)**(-5))*(3*x**2*S-3*x*math.sqrt(v*(1-x**2))*C+(v-1)*(1-x**2)*S)
            } else if (n==4){
                Se=(math.sqrt(1-x**2)**(-7))*(15*x**3*S-15*x**2*math.sqrt(v*(1-x**2))*C+(6*v-9)*x*(1-x**2)*S-(v-4)*math.sqrt(v*(1-x**2)**3)*C)
            } else if (n==5){
                Se= (math.sqrt(1-x**2)**(-9))*(105*x**4*S-105*x**3*math.sqrt(v*(1-x**2))*C+(45*v-90)*x*(1-x**2)*S-(10*v-55)*math.sqrt(v*(1-x**2)**3)*C+(v**2-10*v+9)*(1-x**2)**2*S)
            }
        } else if (v1>=0) {
    ...
    const qet= (n,x)=> {
        ...
        if (v1<0){
            C=math.cosh(math.sqrt(v)*math.acos(x));
            S=(math.exp(math.sqrt(v)*math.acos(x))-math.exp(-math.sqrt(v)*math.acos(x)))/2;
                if (n==0){
                    Set= S
                } else if (n==1){
                    Set= (math.sqrt(1-x**2)**(-1))*C
                } else if (n==2){
                    Set=(math.sqrt(1-x**2)**(-3))*(x*C-math.sqrt(v*(1-x**2))*S)
                } else if (n==3){
                    Set=(math.sqrt(1-x**2)**(-5))*(3*x**2*C-3*x*math.sqrt(v*(1-x**2))*S+(v-1)*(1-x**2)*C)
                } else if (n==4){
                    Set=(math.sqrt(1-x**2)**(-7))*(15*x**3*C-15*x**2*math.sqrt(v*(1-x**2))*S+(6*v-9)*x*(1-x**2)*C-(v-4)*math.sqrt(v*(1-x**2)**3)*S)
                } else if (n==5){
                    Set= (math.sqrt(1-x**2)**(-9))*(105*x**4*C-105*x**3*math.sqrt(v*(1-x**2))*S+(45*v-90)*x*(1-x**2)*C-(10*v-55)*math.sqrt(v*(1-x**2)**3)*S+(v**2-10*v+9)*(1-x**2)**2*C)
                }
        } else if (v1>=0) {
            C=math.cos(math.sqrt(v)*math.acos(x))
            S=math.sin(math.sqrt(v)*math.acos(x))
