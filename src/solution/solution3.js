...
const Solution3=({n, lock, ref0, ref1, ref2, page, setp, dim, setm, modal, setl, setn, setem, m, setdim, tab, setErrorr})=>{
    ...
    const pin=(n,m)=> {
        var t, S, w1, a1
        w1=((factorial(2*n))/(factorial(n)*factorial(n-m-1)))
        a1=findhcf(w1,2**n)
        if ((n-m-1)<=1){
            if ((n-m-1)==0){
            S='\\('+(w1/a1) + '\\)';
            ...
            for (let r = 1; r <= (n-m-1)/2; r++) {
                ...
                if (Number.isInteger(r/2)) {
                    S= S + '\\(\\ +\\ '+ Math.round(((factorial(2*n-2*r))/(factorial(n-r)*factorial(n-m-1-2*r)*factorial(r)))/a1) + t;
                } else {
                    S= S + '\\(\\ -\\ '+ Math.round(((factorial(2*n-2*r))/(factorial(n-r)*factorial(n-m-1-2*r)*factorial(r)))/a1) + t;
                }
            } 
    ...
    const qen1=(m,r)=>{
        ...
        t1===1? t1='':t1=t1
        r===1? t2='': t2="^"+r
        S='\\(\\frac {'+(factorial(m+1)/(factorial(m+1-r)*r))+'} {'+t1+'(1-x^2)'+t2+'}\\)'
        return S
    }
        ...
        if (n-m-1>0){
            ...
            if ((n-m-2)===0){
                ...
                for (let r = 1; r < v1 + 1; r++) {
                    ...
                    S2=refrac(Math.round(factorial(n-1-r)*factorial(r)),Math.round(factorial(2*r+1)*factorial(n-m-2-2*r)))
    ...
    const total =(n,m)=>{
        var S
        n-m-1>0? S= "\\(["+pin(n,m)+"tanh^{-1}(x)"+qin(n,m):S="\\(["+pin(n,m)+"tanh^{-1}(x)"
        for (let r = 1; r <= m+1; r++) {
    ...
    const p= (n,m,x)=> {
        ...
        for (let r = 0; r <= (n-m-1)/2; r++) {
            S+=((-1)**r*factorial(2*n-2*r)*x**(n-m-2*r-1))/(factorial(n-r)*factorial(n-m-2*r-1)*factorial(r));   
        }
        return (((1-x**2)**(m+1))*S)/(2**n)
    }
    ...
    const qet= (n,m,x)=> {
        var S=0
        var S1=0
        for (let r = 0; r <=(n-m-2)/2; r++) {
            for (let i = 0; i < r+1; i++) {
                S1 += (factorial(2*n-2*i)*factorial(2*i))/(factorial(n-i)*factorial(i))**2;
            }
            S +=(-1)**(r+1)*S1*((factorial(n-1-r)*factorial(r)*x**(n-m-2-2*r))/(factorial(2*r+1)*factorial(n-m-2-2*r)))
        }
        return S/2**n
    }
    ...
    const qete =(n,m,x)=>{
        ...
        for (let r=1; r<=m+1; r++) {
            ...
            S += ((factorial(m+1))/(factorial(m+1-r)*r))*S1*(p(n,m-r,x)/((1-x**2)**r))
        }
        return S
    }

    const qe=(n,m,x)=> {
        return  ((1-x**2)**(m+1))*(p(n,m,x)*Math.atanh(x)+ qete(n,m,x)+qet(n,m,x))
    }

    const formap=Array.from(Array(m+2).keys()).slice(1)
    ...
    var layout1={
        title:{
            text: '<span>Graph of((1-x^2)^'+(m+1)+')*P</span><span style="font-size: 0.7em">'+n+'</span><span>(x,'+m+') for -1<x<1<\span>'
        },
        xaxis:{
            title:{
                text: '<span>x<\span>',
        ...
        yaxis:{
            title:{
                text: '<span>((1-x^2)^'+(m+1)+')*P</span><span style="font-size: 0.7em">'+n+'</span><span>(x,'+m+')<\span>',
                font:{
                    color:'rgb(238, 66, 238)'
                }
            },
    ...
    var layout2={
        title:{
            text: '<span>Graph of ((1-x^2)^'+(m+1)+')*Q</span><span style="font-size: 0.7em">'+n+'</span><span>(x,'+m+') for -1<x<1<\span>',
            font:{
                color:'rgb(236, 221, 236)',
                autosize: true
            }
        ...
        yaxis:{
            title:{
                text: '<span>((1-x^2)^'+(m+1)+')*Q</span><span style="font-size: 0.7em">'+n+'</span><span>(x,'+m+')<\span>',
                autosize: true,
                font:{
                    color:'rgb(238, 66, 238)'
                }
    ...
    const coeff='\\((1-x^2)y\'\'+'+2*(m)+'xy\'+'+(n-m)*(n+m+1)+'y=0\\)';
    const coeff1='\\(y\\ =\\ (1-x^2)^{'+(m+1)+'}[C_1\\ P_{'+n+'}(x,'+m+')\\ + C_2\\ Q_{'+n+'}(x,'+m+')]\\)';
    const pp=pin(n,m);
    const qq=qin(n,m);
    const pn="\\(P_{"+n+"}(x,"+m+")=\\)";
    const qn="\\(Q_{"+n+"}(x,"+m+")=\\)";
            ...
            <motion.div className="tab-pane fade show active" id="nav-full-solution" role="tabpanel"
                ...
                <section>
                    <div className="container-lg" id="pdf">
                        ...
                        <div className='beginner fs-5'>
                            ...
                            <div className={(dim.w<700)?
                                    "text-start ms-0 pe-2 fs-5 ps-0 mb-3":"text-start fs-5 mb-3"} style={{height: "fit-content"}}>
                                    <div className="we we3" style={pn.length<24?{textAlign:"start"}:{width:"500px", textAlign:"center"}}>
                    ...
                    <div>
                        ...
                        <div className={(dim.w<700)?
                            "text-start ms-0 pe-2 ps-0 beginner fs-5 my-3":"text-start beginner fs-5 mb-3"} style={{height: "fit-content"}}>
                            <div className={pin(n,m).length<150?"we p we3 text-start":"we p we3 text-center"}>
                                <div className="d-flex align-items-center justify-contents-center mb-2">
                                    <div style={{marginLeft:"0%"}}>{qn}</div>
                                    <span style={{scale:"1.15", marginLeft:da*2+"px", marginRight: da*2+"px"}}>{a2}</span>
                                    <div className="d-flex align-items-center">
                                    <span className={pn.length>150?"npall2 mb-5 py-2 px-1":pp==="\\(1\\)"?"":"npall py-2 npadd"} style={(pin(n,m).length>150)?{ minWidth: "fit-content", textAlign:"center"}:{ textAlign:"center"}}>{pp==="\\(1\\)"?"":pp}</span>
