...
var array=[
    "\\(P_n(x,m)=\\frac {1} {2^n}\\sum_{r=0}^\\frac {n-m-1} {2} \\frac {(-1)^r\\ (2n-2r)!} {(n-r)!\\ (n-m-2r-1)!\\ r!}x^{n-m-2r-1}\\)",
    "\\(Q_n(x,m)=[P_n(x,m)]\\ tanh^{-1}(x) \\ \\ +\\)",
    "\\(\\frac {1} {2^n}\\sum_{r=0}^\\frac {n-m-2} {2}(-1)^{r+1}\\)",
    "\\(\\sum_{k=0}^r \\frac {(2n-2k)!\\ (2k)!}{((n-k)!\\ k!)^2}\\)",
    "\\(\\frac {(n-1-r)!\\ r!}{(2r+1)!\\ (n-m-2r-2)!}x^{n-m-2r-2}\\)",
    "\\(\\sum_{r=1}^m \\frac {(m+1)!} {(m+1-r)!r} \\)",
    "\\(\\sum_{k=0}^\\frac {r-1}{2} \\frac {r!}{(r-2k-1)!(2k+1)!} x^{r-2h-1}\\)",
    "\\(\\frac {P_n(x,m-r)} {(1-x^2)^r} \\ \\ +\\ \\)",
]

const Calculation3=({n, setn,n1,setn1,dim, setm, modal,calc,ref0, m, setem, setcalc, setp, settab, setdim, setErrorr})=>{
  const {w,h}=dim
  ...
  const err=()=>{
    if((n1<0 || Number.isInteger(Number(n1))===false || !n1)){
      ...
      setse(false)
      setErrorr("Note that m must be either zero or a positive integer less than the value of n.")
    } else if (Number(m)>=Number(n)){
      setse(false)
      setErrorr("Note that m must be less than the value of n.")
    } else {
      setse(true)
      setcalc(false)
                    ...
                    <div id="pa1">
                        ...
                        <AnimatePresence>
                            <motion.div key={n1}
                              ...
                              exit={{opacity:0}}
                            >
                              <div className="container table-responsive pt-0" style={(!n1 || n1<0 || Number.isInteger(Number(n1))===false||(!m)||Number.isInteger(Number(m))==false || Number(m)<0 || Number(m)>=Number(n) || n1-m>35 || m>15)?
                                ...
                                >
                                  ...
                                  (!n1 || n1<0 || Number.isInteger(Number(n1))===false||(!m)||Number.isInteger(Number(m))==false || Number(m)<0 || Number(m)>=Number(n) || n1-m>35 || m>15)?
                                  "\\((1-x^2)y''+2mxy'+(n-m)(n+m+1)y=0\\)":`\\((1-x^2)y''+${2*(m*1)}xy'+${((n*1)-(m*1))*((n*1)+(m*1)+1)}y=0\\)`}</span>
                                </a>
                                </motion.div>
                              </div>
                            </motion.div>
                          {
                            (!select) && (
                              <motion.div key="selecte"
                                ...
                                exit="exit2"
                              >
                                <h2><span className={dim.w<700?"text-start beginner text-muted":"text-start fs-5 text-muted"}> Select the value of m and n; note that n <span>\(\gt m\)</span> 
                                <span className="ms-2">
                                ...
                                <div className="row align-items-center text-center justify-content-center">
                                  ...
                                  <input ref={ref} className="col-lg-7 col-10 text-center px-1" type="number" onInput={()=>setcalc(false)} id="input" placeholder="Enter m"
                                    onChange={(e)=>{setem(e.target.value)}} value={m}/>
                                  </div>
                                  <div className="col-3 note1 align-items-center">
                                  <Link to={(!m || !n1 || m>=n)?"": "/solution3"}><motion.button variants={Variants} whileHover="hover" className="btn textpele px-2 col-lg-2 col-3" id="btn--select" onClick={err}>Proceed</motion.button></Link>
                                  </div>
                                  <div className="col-4 note1 align-items-end">
                                  <input className="col-lg-7 col-10 text-center px-1" type="number" onInput={()=>setcalc(false)} id="input" placeholder="Enter n"
                  ...
                  <div className="col-12 col-lg-7 beginner" style={w>350?{margin: "auto"}: {width:"100%"}}>
                    ...
                    <header id="logo" className="textpep py-3 beginner">
                      <div className="container table-responsive pt-0" style={{width:"fit-content", overflowX:"hidden", overflowY:"hidden"}}>
                        ...
                        >
                          \((1-x^2)y''+2mxy'+(n-m)(n+m+1)y=0
                          \)
                        </motion.div>
                      </div>
                      <div className="left1">\( where\ n=0,1,2,3,... \ \ and\ m\lt n\)</div>
                      <div>{"\\(y=(1-x^2)^{m+1}[C_1\ P_n(x,m)+ C_2\ Q_n(x,m)]\\)"}</div>
                      <div className="left1">\( where: \)</div>
